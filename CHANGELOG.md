...
- Replaced public references to the historical six-question/twenty-four-observation battery as the current commercial requirement.
- Simplified Compare & Monitor documentation around one fixed primary-company context and up to three selected competitors; removed the retired customer-facing per-domain credit/reuse purchase model.
- Updated Unified Report entitlement language to Review/subscription grants and preserved repair entitlement for system-caused invalid outcomes.
- Aligned the documentation README with the current six-stage public workflow.
- Changed documentation maintenance from a separate daily-reminder model to event-driven updates with periodic drift review as a backstop.
- Marked the old Professional Deep Investigation page historical/experimental rather than an active plan capability.

### Preserved methodology boundaries

- reachable != visible;
- visible != correctly interpreted;
- unsupported != hallucinated;
- direct measured answer != later grounded verification;
- first valid observation != drift;
- helper-file presence != proof of downstream AI use or causation.
